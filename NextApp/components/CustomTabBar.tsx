...
function isRouteVisible(routeName: string, isCourier: boolean): boolean {
  if (routeName === 'index' || routeName === 'two') return !isCourier;
...
export const CustomTabBar: React.FC<BottomTabBarProps> = React.memo(
  ({ state, navigation }) => {
    ...
    const visibleRoutes = state.routes.filter((r) => isRouteVisible(r.name, isCourier));

    return (
      <View
        pointerEvents="box-none"
        style={[styles.wrapper, { bottom: insets.bottom > 0 ? insets.bottom + 8 : 16 }]}>
        <View style={styles.shadowWrap}>
          <View style={styles.barOuter}>
            ...
            <LinearGradient
              pointerEvents="none"
              colors={[
                theme.mode === 'dark' ? 'rgba(255,255,255,0.16)' : 'rgba(255,255,255,0.55)',
                'transparent',
              ]}
              start={{ x: 0.5, y: 0 }}
              end={{ x: 0.5, y: 0.45 }}
            ...
            <View
              ...
              style={[
                StyleSheet.absoluteFillObject,
                {
                  ...
                  borderColor:
                    theme.mode === 'dark' ? 'rgba(255,255,255,0.14)' : 'rgba(255,255,255,0.7)',
                },
              ]}
            />
...
const TabItem: React.FC<{
...
}> = ({ isFocused, onPress, iconActive, iconInactive, badge, theme, styles }) => {
  ...
  const onPressIn = () =>
    Animated.spring(press, { toValue: 0.86, useNativeDriver: true, ...motion.springSnappy }).start();
  const onPressOut = () =>
    Animated.spring(press, { toValue: 1, useNativeDriver: true, ...motion.spring }).start();

  const iconScale = focused.interpolate({ inputRange: [0, 1], outputRange: [1, 1.08] });

  return (
    <Pressable onPress={onPress} onPressIn={onPressIn} onPressOut={onPressOut}>
      <Animated.View style={[styles.item, { transform: [{ scale: press }] }]}>
        <Animated.View style={[StyleSheet.absoluteFill, styles.itemPillWrap, { opacity: focused }]}>
          <LinearGradient
        ...
        <Animated.View style={[styles.iconWrapper, { transform: [{ scale: iconScale }] }]}>
          ...
          />
          {badge > 0 && (
            <View style={[styles.badge, { borderColor: isFocused ? '#fff' : theme.surface }]}>
