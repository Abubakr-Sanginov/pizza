...
export const Stories: React.FC<Props> = ({ className }) => {
  ...
  return (
    <>
      <Container
        className={cn(
          "flex items-center justify-between gap-2 my-10 overflow-x-auto scrollbar-hide p-2",
          className,
        )}
      >
        {isLoading &&
          [...Array(6)].map((_, index) => (
            <div
              key={index}
              className="w-[150px] md:w-[200px] h-[200px] md:h-[250px] bg-muted rounded-md animate-pulse flex-shrink-0"
            />
          ))}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed left-0 top-0 w-full h-full bg-black/90 flex items-center justify-center z-[100] p-4"
            ...
            >
              ...
              <div className="rounded-2xl overflow-hidden shadow-2xl border border-white/10">
                {selectedStory && selectedStory.items.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center w-full h-[80vh] bg-zinc-900 rounded-2xl text-white/50 text-sm">
                    Нет контента
                  </div>
                )}
              </div>
