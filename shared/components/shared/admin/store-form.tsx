...
export const StoreForm: React.FC<Props> = ({ initialData }) => {
  ...
  const form = useForm<StoreFormValues>({
    ...
    defaultValues: {
      ...
      ownerUsername: initialData?.botUsers?.[0]?.username || '',
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="grid grid-cols-2 gap-5">
            <div>
              ...
              <Input {...form.register('lat')} placeholder="38.559..." step="any" type="number" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Долгота (Lng)</label>
              <Input {...form.register('lng')} placeholder="68.774..." step="any" type="number" />
            </div>
          </div>
